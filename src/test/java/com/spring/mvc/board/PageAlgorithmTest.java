...
public class PageAlgorithmTest {
	 ...
	 * 한 화면당 보여질 페이지 수
	 ...
	 - 그리고 이 사람은 31페이지를 현재 보고 있습니다.
	 - 그리고 한 화면에 버튼은 10개가 배치됩니다.
	 - 그렇다면 위 공식에 의한 끝 페이지는 몇 번으로 계산되는가? -> 40번
	 - 하지만 실제 끝 페이지는 몇 번에서 끝나는가? -> 33번

	 # 5-1. 이전 버튼 활성 여부 설정.
	 - 언제 이전버튼을 활성화/비활성화 시킬 것인가의 여부 -> 시작페이지가 1인 부분에서 비활성.
	 공식: 시작 페이지 번호가 1로 구해진 시점에서 비활성 처리. 나머지는 활성.
	...
	public void pagingAlgorithmTest() {
		...
		//총 게시물 구하는 수 테스트
		System.out.println("-------------------------");
		//System.out.println("# 총 게시물 수: " + mapper.countArticles(search));
		System.out.println("-------------------------");
		...
		//다음 버튼 활성, 비활성 여부
		boolean isNext = (mapper.countArticles(search) <= (endPage * paging.getCountPerPage())) ? false : true;
		System.out.println("다음 버튼 활성화 여부: " + isNext);

		//끝 페이지 보정
		if(!isNext) {
			endPage = (int) Math.ceil(mapper.countArticles(search) / (double)paging.getCountPerPage());
		}
	...
	public void searchTest() {
		...
		page.setPage(4);
		search.setKeyword("9");
		search.setCondition("content");

		System.out.println("====================================================");
		mapper.getArticleList(page, search).forEach(vo-> System.out.println(vo));
	}
}
